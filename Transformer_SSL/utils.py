...
def save_checkpoint(config, epoch, model, max_accuracy, optimizer, lr_scheduler, logger):
    save_state = {'model': model.state_dict(),
                ...
                'epoch': epoch,
                'config': map_nested_dicts(config, str), # some entries might be None
                 }
    ...
    if logger is not None: logger.info(f"{save_path} saving......")
    torch.save(save_state, save_path)
    torch.save(save_state, os.path.join(config.OUTPUT, f'checkpoint.pth'))
    if logger is not None: logger.info(f"{save_path} saved !!!")
