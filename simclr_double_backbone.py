...
class SimCLRDoubleBackbone(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.use_logging: logging.basicConfig(filename=os.path.join(self.writer.log_dir, 'training.log'), level=logging.DEBUG)
    ...
    def train(self, train_loader):
            ...
            for sample in pbar:
                ...
                if n_iter % self.args.log_every_n_steps == 0:
                    ...
                    if self.use_logging:
                        ...
                        self.writer.add_scalar('acc/top1', mean_top1, global_step=n_iter)
                        self.writer.add_scalar('acc/top5', mean_top5, global_step=n_iter)
                    ...
                    wandb.log({"loss" : mean_loss,
                              ...
                              "epoch" : epoch_counter}, step=n_iter)
                ...
                #n_iter += 1
                n_iter += s1.shape[0] # count the number of processed samples (i.e. batch_size * steps)
                pbar.set_description(f"Epoch:{epoch_counter}, Step:{n_iter}, Loss:{np.mean(loss_per_logging[-100:]):.4}") #"{epoch_accuracy[-100:].mean():.4}")

            if epoch_counter % 25 == 0:
                print("Saving checkpoint for epoch:", epoch_counter)
                checkpoint_name = "checkpoints/simclr-double-backbone-" + self.run_name + "-epoch" + str(epoch_counter) + ".pth"
                save_checkpoint({
                'epoch': epoch_counter,
                'arch': self.args.arch,
        ...
        if self.use_logging: logging.info("Training has finished.")
        ...
        if self.use_logging: logging.info(f"Model checkpoint and metadata has been saved at {self.writer.log_dir}.")
